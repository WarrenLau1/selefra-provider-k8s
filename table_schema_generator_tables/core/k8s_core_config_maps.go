...
type TableK8sCoreConfigMapsGenerator struct {
}

var _ table_schema_generator.TableSchemaGenerator = &TableK8sCoreConfigMapsGenerator{}

func (x *TableK8sCoreConfigMapsGenerator) GetTableName() string {
	return "k8s_core_config_maps"
}

func (x *TableK8sCoreConfigMapsGenerator) GetTableDescription() string {
	return ""
}

func (x *TableK8sCoreConfigMapsGenerator) GetVersion() uint64 {
	return 0
}

func (x *TableK8sCoreConfigMapsGenerator) GetOptions() *schema.TableOptions {
	return &schema.TableOptions{}
}

func (x *TableK8sCoreConfigMapsGenerator) GetDataSource() *schema.DataSource {
	return &schema.DataSource{
		Pull: func(ctx context.Context, clientMeta *schema.ClientMeta, client any, task *schema.DataSourcePullTask, resultChannel chan<- any) *schema.Diagnostics {
			cl := client.(*k8s_client.Client).Client().CoreV1().ConfigMaps("")
...
func (x *TableK8sCoreConfigMapsGenerator) GetExpandClientTask() func(ctx context.Context, clientMeta *schema.ClientMeta, client any, task *schema.DataSourcePullTask) []*schema.ClientTaskContext {
	return k8s_client.ContextExpand()
}

func (x *TableK8sCoreConfigMapsGenerator) GetColumns() []*schema.Column {
	return []*schema.Column{
		...
		table_schema_generator.NewColumnBuilder().ColumnName("binary_data").ColumnType(schema.ColumnTypeJSON).
			Extractor(column_value_extractor.StructSelector("BinaryData")).Build(),
	}
}

func (x *TableK8sCoreConfigMapsGenerator) GetSubTables() []*schema.Table {
	return nil
}
