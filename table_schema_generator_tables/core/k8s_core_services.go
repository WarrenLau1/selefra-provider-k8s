...
func (x *TableK8sCoreServicesGenerator) GetDataSource() *schema.DataSource {
	return &schema.DataSource{
		Pull: func(ctx context.Context, clientMeta *schema.ClientMeta, client any, task *schema.DataSourcePullTask, resultChannel chan<- any) *schema.Diagnostics {
			services := client.(*k8s_client.Client).K8sServices().K8sServices
...
func (x *TableK8sCoreServicesGenerator) GetColumns() []*schema.Column {
	return []*schema.Column{
		...
		table_schema_generator.NewColumnBuilder().ColumnName("spec_health_check_node_port").ColumnType(schema.ColumnTypeInt).
		...
		table_schema_generator.NewColumnBuilder().ColumnName("spec_cluster_ip").ColumnType(schema.ColumnTypeIp).
			Extractor(column_value_extractor.StructSelector("Spec.ClusterIP")).Build(),
		table_schema_generator.NewColumnBuilder().ColumnName("spec_cluster_ips").ColumnType(schema.ColumnTypeIpArray).
		...
		table_schema_generator.NewColumnBuilder().ColumnName("spec_load_balancer_ip").ColumnType(schema.ColumnTypeIp).
		...
		table_schema_generator.NewColumnBuilder().ColumnName("spec_external_ips").ColumnType(schema.ColumnTypeIpArray).
