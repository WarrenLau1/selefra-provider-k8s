...
type TableK8sCoreEventsGenerator struct {
}

var _ table_schema_generator.TableSchemaGenerator = &TableK8sCoreEventsGenerator{}

func (x *TableK8sCoreEventsGenerator) GetTableName() string {
	return "k8s_core_events"
}

func (x *TableK8sCoreEventsGenerator) GetTableDescription() string {
	return ""
}

func (x *TableK8sCoreEventsGenerator) GetVersion() uint64 {
	return 0
}

func (x *TableK8sCoreEventsGenerator) GetOptions() *schema.TableOptions {
	return &schema.TableOptions{}
}

func (x *TableK8sCoreEventsGenerator) GetDataSource() *schema.DataSource {
	return &schema.DataSource{
		Pull: func(ctx context.Context, clientMeta *schema.ClientMeta, client any, task *schema.DataSourcePullTask, resultChannel chan<- any) *schema.Diagnostics {
			cl := client.(*k8s_client.Client).Client().CoreV1().Events("")
...
func (x *TableK8sCoreEventsGenerator) GetExpandClientTask() func(ctx context.Context, clientMeta *schema.ClientMeta, client any, task *schema.DataSourcePullTask) []*schema.ClientTaskContext {
	return k8s_client.ContextExpand()
}

func (x *TableK8sCoreEventsGenerator) GetColumns() []*schema.Column {
	return []*schema.Column{
		...
		table_schema_generator.NewColumnBuilder().ColumnName("first_timestamp").ColumnType(schema.ColumnTypeTimestamp).
			Extractor(column_value_extractor.StructSelector("FirstTimestamp")).Build(),
		table_schema_generator.NewColumnBuilder().ColumnName("related").ColumnType(schema.ColumnTypeJSON).
			Extractor(column_value_extractor.StructSelector("Related")).Build(),
		...
		table_schema_generator.NewColumnBuilder().ColumnName("last_timestamp").ColumnType(schema.ColumnTypeTimestamp).
			Extractor(column_value_extractor.StructSelector("LastTimestamp")).Build(),
		...
		table_schema_generator.NewColumnBuilder().ColumnName("action").ColumnType(schema.ColumnTypeString).
			Extractor(column_value_extractor.StructSelector("Action")).Build(),
		table_schema_generator.NewColumnBuilder().ColumnName("reporting_instance").ColumnType(schema.ColumnTypeString).
			Extractor(column_value_extractor.StructSelector("ReportingInstance")).Build(),
		...
		table_schema_generator.NewColumnBuilder().ColumnName("reason").ColumnType(schema.ColumnTypeString).
			Extractor(column_value_extractor.StructSelector("Reason")).Build(),
		...
		table_schema_generator.NewColumnBuilder().ColumnName("reporting_component").ColumnType(schema.ColumnTypeString).
			Extractor(column_value_extractor.StructSelector("ReportingController")).Build(),
		...
		table_schema_generator.NewColumnBuilder().ColumnName("involved_object").ColumnType(schema.ColumnTypeJSON).
			Extractor(column_value_extractor.StructSelector("InvolvedObject")).Build(),
		table_schema_generator.NewColumnBuilder().ColumnName("series").ColumnType(schema.ColumnTypeJSON).
			Extractor(column_value_extractor.StructSelector("Series")).Build(),
	}
}

func (x *TableK8sCoreEventsGenerator) GetSubTables() []*schema.Table {
	return nil
}
