...
func (x *TableK8sAppsStatefulSetsGenerator) GetDataSource() *schema.DataSource {
	return &schema.DataSource{
		Pull: func(ctx context.Context, clientMeta *schema.ClientMeta, client any, task *schema.DataSourcePullTask, resultChannel chan<- any) *schema.Diagnostics {
			cl := client.(*k8s_client.Client).K8sServices().StatefulSets
...
func (x *TableK8sAppsStatefulSetsGenerator) GetColumns() []*schema.Column {
	return []*schema.Column{
		...
		table_schema_generator.NewColumnBuilder().ColumnName("status_current_replicas").ColumnType(schema.ColumnTypeInt).
