...
func (x *TableK8sAppsDaemonSetsGenerator) GetDataSource() *schema.DataSource {
	return &schema.DataSource{
		Pull: func(ctx context.Context, clientMeta *schema.ClientMeta, client any, task *schema.DataSourcePullTask, resultChannel chan<- any) *schema.Diagnostics {
			cl := client.(*k8s_client.Client).K8sServices().DaemonSets
...
func (x *TableK8sAppsDaemonSetsGenerator) GetColumns() []*schema.Column {
	return []*schema.Column{
		...
		table_schema_generator.NewColumnBuilder().ColumnName("status_number_misscheduled").ColumnType(schema.ColumnTypeInt).
			Extractor(column_value_extractor.StructSelector("Status.NumberMisscheduled")).Build(),
		table_schema_generator.NewColumnBuilder().ColumnName("status_current_number_scheduled").ColumnType(schema.ColumnTypeInt).
			Extractor(column_value_extractor.StructSelector("Status.CurrentNumberScheduled")).Build(),
		table_schema_generator.NewColumnBuilder().ColumnName("status_desired_number_scheduled").ColumnType(schema.ColumnTypeInt).
		...
		table_schema_generator.NewColumnBuilder().ColumnName("status_number_available").ColumnType(schema.ColumnTypeInt).
		...
		table_schema_generator.NewColumnBuilder().ColumnName("status_updated_number_scheduled").ColumnType(schema.ColumnTypeInt).
		...
		table_schema_generator.NewColumnBuilder().ColumnName("status_number_ready").ColumnType(schema.ColumnTypeInt).
			Extractor(column_value_extractor.StructSelector("Status.NumberReady")).Build(),
		table_schema_generator.NewColumnBuilder().ColumnName("status_number_unavailable").ColumnType(schema.ColumnTypeInt).
