...
func (x *TableK8sBatchCronJobsGenerator) GetDataSource() *schema.DataSource {
	return &schema.DataSource{
		Pull: func(ctx context.Context, clientMeta *schema.ClientMeta, client any, task *schema.DataSourcePullTask, resultChannel chan<- any) *schema.Diagnostics {
			jobs := client.(*k8s_client.Client).K8sServices().CronJobs
	...
	return []*schema.Column{
		...
		table_schema_generator.NewColumnBuilder().ColumnName("status_last_schedule_time").ColumnType(schema.ColumnTypeTimestamp).
			Extractor(column_value_extractor.WrapperExtractFunction(func(ctx context.Context, clientMeta *schema.ClientMeta, client any, task *schema.DataSourcePullTask, row *schema.Row, column *schema.Column, result any) (any, *schema.Diagnostics) {
				...
				value := ognl.Get(result, "Status.LastScheduleTime").Value()
				if reflect_util.IsNil(result) {
					return nil, nil
				}
		...
		table_schema_generator.NewColumnBuilder().ColumnName("spec_successful_jobs_history_limit").ColumnType(schema.ColumnTypeInt).
		...
		table_schema_generator.NewColumnBuilder().ColumnName("status_last_successful_time").ColumnType(schema.ColumnTypeTimestamp).
			Extractor(column_value_extractor.WrapperExtractFunction(func(ctx context.Context, clientMeta *schema.ClientMeta, client any, task *schema.DataSourcePullTask, row *schema.Row, column *schema.Column, result any) (any, *schema.Diagnostics) {
				...
				value := ognl.Get(result, "Status.LastSuccessfulTime").Value()
				if reflect_util.IsNil(result) {
					return nil, nil
				}
				of := reflect.ValueOf(value)
		...
		table_schema_generator.NewColumnBuilder().ColumnName("spec_starting_deadline_seconds").ColumnType(schema.ColumnTypeInt).
		...
		table_schema_generator.NewColumnBuilder().ColumnName("spec_failed_jobs_history_limit").ColumnType(schema.ColumnTypeInt).
