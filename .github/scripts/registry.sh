...
for f in $FOR; do
  ...
  sed "${sedi[@]}" "s#{{.CheckSumLinuxARM64}}#${darwin_arm64}#g" provider/k8s/$version/supplement.yaml
  sed "${sedi[@]}" "s#{{.CheckSumLinuxAMD64}}#${darwin_amd64}#g" provider/k8s/$version/supplement.yaml
  sed "${sedi[@]}" "s#{{.CheckSumWindowsARM64}}#${windows_amd64}#g" provider/k8s/$version/supplement.yaml
  sed "${sedi[@]}" "s#{{.CheckSumWindowsAMD64}}#${linux_amd64}#g" provider/k8s/$version/supplement.yaml
  sed "${sedi[@]}" "s#{{.CheckSumDarwinARM64}}#${linux_arm64}#g" provider/k8s/$version/supplement.yaml
  sed "${sedi[@]}" "s#{{.CheckSumDarwinAMD64}}#${windows_arm64}#g" provider/k8s/$version/supplement.yaml
done
